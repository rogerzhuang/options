...
def print_summary(portfolio_df, pair_results):
    ...
    print("\n=== Backtest Summary ===")
    print(f"Period: {portfolio_df.index[0].date()} to {portfolio_df.index[-1].date()}")
    print(f"Total Trading Days: {len(portfolio_df)}")
    
    # Calculate and print average weights
    print("\n=== Average Portfolio Weights ===")
    weight_cols = [col for col in portfolio_df.columns if col.endswith('_weight')]
    if weight_cols:
        avg_weights = portfolio_df[weight_cols].mean()
        print("\nInverse Volatility Weights:")
        for col in weight_cols:
            pair_name = col.replace('_weight', '')
            print(f"  {pair_name}: {avg_weights[col]:.2%}")
